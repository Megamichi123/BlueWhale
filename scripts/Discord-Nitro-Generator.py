...
try:
    Slow(discord_banner)
    webhook = input(
        f"{BEFORE + current_time_hour() + AFTER} {INPUT} Webhook ? (y/n) -> {reset}"
    )
    ...
    def nitro_check():
        ...
        if response.status_code == 200:
            ...
            print(
                f"{BEFORE_GREEN + current_time_hour() + AFTER_GREEN} {GEN_VALID} Status:  {white}Valid{green}  Nitro: {white}{url_nitro}{reset}"
            )
        else:
            print(
                f"{BEFORE + current_time_hour() + AFTER} {GEN_INVALID} Status: {white}Invalid{blue} Nitro: {white}{url_nitro}{reset}"
            )
